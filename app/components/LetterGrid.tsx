...
const buildGrid = (container: HTMLDivElement | null) => {
    ...
    const paddingY = container ? parseFloat(getComputedStyle(container).paddingTop) * 2 : VERTICAL_PADDING;
    const paddingX = container ? parseFloat(getComputedStyle(container).paddingLeft) * 2 : HORIZONTAL_PADDING;
    ...
    projects.forEach((project, index) => {
        ...
        for (const row of shuffledRows) {
            ...
            const starts = Array.from({length: maxStart + 1}, (_, i) => i).sort(() => Math.random() - 0.5);
            const start = starts.find((s) => Array.from({length: project.title.length}, (_, letterIndex) => rows[row][s + letterIndex]).every((cell) => !cell.href));
